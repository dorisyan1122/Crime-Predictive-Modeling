...
rmse_lasso_v<-rmse(lasso_v_predictions_df, actual, lasso_v_predictions)
```
...
rmse_rf_v<-rmse(rf_v_predictions_df, actual, rf_v_predictions)
```
...
rmse_xgb_v<-rmse(xgb_v_predictions_df, actual, xgb_v_predictions)
```

## Final comparison of rmse

```{r}
bind_rows(`Violent Crime Ridge regression`= rmse_lasso_v, 
          `Violent Crime Lasso regression`= rmse_ridge_v, 
          `Violent Crime Random Forest` = rmse_rf_v, 
          `Violent Crime MARS` = rmse_mars_v,
          `Violent Crime XG Boost`= rmse_xgb_v)
```
...
ridge_p_predictions_df<-
  tibble(county,actual,ridge_p_predictions)

rmse_ridge_p<-rmse(ridge_p_predictions_df, actual, ridge_p_predictions)
```
...
lasso_p_predictions_df<-
  tibble(county,actual,lasso_p_predictions)

rmse_lasso_p<-rmse(lasso_p_predictions_df, actual, lasso_p_predictions)
```
...
rmse_rf_p<-rmse(rf_p_predictions_df, actual, rf_p_predictions)
```
...
Based on the result from random forest model, the top 15 important variables for predicting property crime are above Most of the variables are from the socioeconomic data and few are from the land cover data.
...
mars_p_predictions_df<-
  tibble(county,actual,mars_p_predictions)

rmse_mars_p<-rmse(mars_p_predictions_df, actual, mars_p_predictions)
```
...
rmse_xgb_p<-rmse(xgb_p_predictions_df, actual, xgb_p_predictions)
```

## Final comparison of rmse

```{r}
bind_rows(`Violent Crime Ridge Regression`= rmse_lasso_p, 
          `Violent Crime Lasso Regression`= rmse_ridge_p, 
          `Violent Crime Random Forest` = rmse_rf_p, 
          `Violent Crime MARS` = rmse_mars_p,
          `Violent Crime XG Boost`= rmse_xgb_p)
```
...
rmse_xgb_v_2016
```
Final prediction for violent crime rate in 2016 has a RMSE of 0.5927494.
...
rmse_rf_p_2016
```
Final prediction for property crime rate in 2016 has a RMSE of 0.4592552
